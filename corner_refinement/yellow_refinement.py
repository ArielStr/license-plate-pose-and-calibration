...
from .methods.yellow_exit_ransac import corners_from_yellow_exit_ransac
...
def refine_yellow_inner_corners(
    ...
    dynamic_result = estimate_dynamic_yellow_hsv(
        crop
    )
    ...
    yellow_exit_ransac_points = None
    ...
    if method == "mask_lines":
        ...
        if line_points is not None:
            print(
                "Corner method candidate: mask_lines"
            )
        else:
            print(
                "Corner method candidate: mask_lines failed"
            )

    if method == "robust_mask_lines":
        robust_line_points = (
            corners_from_robust_mask_lines(
                largest,
                x_offset,
                y_offset,
                crop=crop,
        ...
        if robust_line_points is not None:
            print(
                "Corner method candidate: robust_mask_lines"
            )
        else:
            print(
                "Corner method candidate: robust_mask_lines failed"
            )

    if method == "yellow_exit_ransac":
        yellow_exit_ransac_points = (
            corners_from_yellow_exit_ransac(
                largest,
                x_offset,
                y_offset,
                crop=crop,
        ...
        if yellow_exit_ransac_points is not None:
            print(
                "Corner method candidate: yellow_exit_ransac"
            )
        else:
            print(
                "Corner method candidate: yellow_exit_ransac failed"
            )
    ...
    print(
        f"Contour area: {cv2.contourArea(largest):.1f} px"
    )
    print(
        f"Width : {max(w, h):.2f} px"
    )
    print(
        f"Height: {min(w, h):.2f} px"
    )
    print(
        f"Aspect ratio: {ratio:.4f}"
    )
    print(
        "=========================================\n"
    )
    ...
    if method == "min_area_rect":
        ...
        if robust_line_points is None:
            raise RuntimeError(
                "robust_mask_lines failed"
            )
        image_points = robust_line_points

    elif method == "yellow_exit_ransac":
        if yellow_exit_ransac_points is None:
            raise RuntimeError(
                "yellow_exit_ransac failed"
            )
        image_points = yellow_exit_ransac_points
    ...
    # Existing generic visualization does not know about the internals of
    # yellow_exit_ransac. It can still draw the selected final points.
    if debug:
